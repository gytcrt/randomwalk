...
This formula computes total API spend by taking the total tokens \(T\), applying a blended per-token price based on the input/output mix (r vs. (1-r))) and their respective rates (P_input) and \(P\_output\), then applying the assumed discount by multiplying by (1-d).
